...
export default async function handler(req: NextApiRequest, res: NextApiResponse<Resp>) {
  try {
    ...
    const safeDays = typeof days === "number" && Number.isFinite(days) ? Math.floor(days) : 30;

    if (!safeUserId) return res.status(400).json({ ok: false, error: "Missing userId" });
    if (safeDays <= 0 || safeDays > 365) return res.status(400).json({ ok: false, error: "Invalid days" });
